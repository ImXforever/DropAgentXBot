...
def test_identity_rl_hash_stable():
    ...
    import hashlib
    expected = int.from_bytes(hashlib.blake2b(b'user:1|ip:2|dev:3', digest_size=4).digest(), 'big') % 100000
    assert v1 == expected
...
# ─── هات‌فیکس 0.6.1 (از لاگ Railway) ───

def test_no_missing_stdlib_imports():
    """هات‌فیکس لاگ: admin.py از time استفاده می‌کرد بدون import → NameError در
    «کاربران». این تست کل هندلرها و ریشه را برای importهای گمشده اسکن می‌کند."""
    import ast
    import glob
    std = {'time', 'json', 're', 'random', 'base64', 'hashlib', 'io', 'math',
           'html', 'tempfile', 'string', 'secrets', 'unicodedata', 'uuid',
           'traceback', 'collections', 'itertools', 'functools', 'textwrap'}
    for f in glob.glob(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'handlers', '*.py')) + \
             glob.glob(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), '*.py')):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Attribute) and isinstance(node.value, ast.Name):
                used.setdefault(node.value.id, []).append(node.lineno)
        for mod, lines in used.items():
            if mod in std and mod not in imported:
                real = [L for L in lines
                        if f"import {mod}" not in src.split('\n')[L - 1]]
                assert not real, f"{f}: «{mod}» import نشده ولی خط {real[:4]} استفاده شده"


def test_campaign_tips_respect_200_char_limit():
    """هات‌فیکس لاگ: callback.answer سقف ۲۰۰ کاراکتر دارد — هندلر باید متن بلند
    را پیام بفرستد نه alert."""
    base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    src = open(os.path.join(base, 'handlers', 'admin.py'), encoding='utf-8').read()
    i = src.index('async def campaign_hints')
    block = src[i:i + 4000]
    assert 'len(tip) <= 190' in block, 'گارد طول tip نیست'
    assert 'callback.message.answer(tip)' in block
