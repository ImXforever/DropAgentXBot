...
async def db_log(logger_name: str, msg: str, *, user_id: int | None = None,
...
async def prune_app_logs(days: int = 14, max_rows: int = 50000) -> int:
    """v3.3.3: نگهداری app_logs — حذف قدیمی‌تر از N روز + کپ سقف ردیف‌ها (ضد رشد بی‌سقف)."""
    ...
    try:
        ...
        async with raw_db() as db:
            if days > 0:
                cur = await db.execute("DELETE FROM app_logs WHERE ts < ?",
                                       (time.time() - days * 86400,))
                removed += cur.rowcount or 0
            if max_rows > 0:
                cur = await db.execute(
                    "DELETE FROM app_logs WHERE rowid IN "
                    "(SELECT rowid FROM app_logs ORDER BY rowid DESC LIMIT -1 OFFSET ?)",
                    (max_rows,))
                removed += cur.rowcount or 0
            await db.commit()
        return removed
    except Exception:
        logger.exception("prune_app_logs failed")
        return 0
