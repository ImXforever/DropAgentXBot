...
_BLOCK_PATTERNS = [
    ...
    re.compile(r"curl\s+.*\|\s*(sh|bash)", re.IGNORECASE),   # curl | sh
...
_SUSPICIOUS_PATTERNS = [
    ...
    re.compile(r"\[INST\]|<\/s>|<\|im_start\|>", re.IGNORECASE),         # LLM fence-markers
    re.compile(r"\b(system|user|assistant)\s*:\s*", re.IGNORECASE),      # role-spoofing
    re.compile(r"(api[_-]?key|password|secret|token)\b", re.IGNORECASE),
    re.compile(r"\b(http|https)://\S+", re.IGNORECASE),                    # unbounded URL fetch
    re.compile(r"\sssh\s|scp\s+", re.IGNORECASE),
]
