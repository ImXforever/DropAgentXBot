...
_KNOWN_PATTERNS = [
    re.compile(r"^\s*(diff|index -{3}|--- \w+|\+\+\+ \w+)", re.MULTILINE),      # git diff
    re.compile(r"^\s*(commit \w+|Author:|Date:)\b", re.MULTILINE),              # git log
    re.compile(r"^\s*On branch |^\s*master|^\s*main", re.MULTILINE),            # git status
    re.compile(r"^\s*(\d+)\s+(passed|failed|ok|error)\b", re.MULTILINE),        # test output
    re.compile(r"^\s*\S+\/\d+\s+\S+\s+\S+\s+", re.MULTILINE),                    # `find`/`ls -l`
    re.compile(r"^\s*(File|Exception|Traceback|  File \")\b", re.MULTILINE),    # tracebacks
    re.compile(r"^\s*(Compiling|Building|Downloading|Installing|Collecting)", re.MULTILINE),  # build logs
]
